...
## step 1 :
datTraits = sample_group
rownames(datTraits) = datTraits$sample
datTraits["subtype"] = ifelse(datTraits$group == "AF",1,0)
head(datTraits)
datExpr0 = t(exp)
...
datExpr = datExpr0[,order(apply(datExpr0,2,mad), decreasing = T)[1:(20188*0.25)]]
nGenes = ncol(datExpr)
nSamples = nrow(datExpr)



flag = identical(rownames(datTraits),rownames(datExpr))
if (!flag) {
  sampleNames = rownames(datExpr);
  datTraits = datTraits[rownames(datTraits) %in% sampleNames,]
  traitRows = match(sampleNames, rownames(datTraits))        # gsm为样本名
  rownames(datTraits) = datTraits[traitRows, 1]
}
datTraits = datTraits[,-1]
datTraits = data.frame(subtype = datTraits$subtype,row.names = rownames(datTraits))
...
pdf("../figure/module_trait.pdf",width = 6,height = 8)
...
if(T){
  # Select module
  module = c("red","green","black");
  # Select module probes
  probes = colnames(datExpr) ## 我们例子里面的probe就是基因
  inModule = is.finite(match(moduleColors, module));
  modProbes = probes[inModule]; 
  head(modProbes)
  write.csv(modProbes,file = "../data/wgcna_genes.csv")
  # 如果使用WGCNA包自带的热图就很丑。
  which.module="brown";
